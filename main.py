...
class Fossil3DReconstructor:
    ...
    def optimize_deformation(
        ...
        print(f"  Scale: {best_scale:.4f}, Loss={loss_history[-1]:.4f}")
            ...
            for i in tqdm(range(max_iters), desc=label):
                ...
                if i >= EARLY_STOP_WINDOW:
                    ...
                    if imp < EARLY_STOP_DELTA:
                        tqdm.write(f"  {label}: early stop at iter {i+1}")
                        if progress_callback:
        ...
        p1_done = run_deform_phase(r1, o1, None, phase1_max, "Phase 1 (coarse)", 0)
        ...
        run_deform_phase(r2, o2, s2, phase2_max, "Phase 2 (fine)", p1_done)
        ...
        print(f"Done: {len(loss_history)} iters, scale={best_scale:.3f}, "
              f"{elapsed:.1f} s, IoU={1.0 - loss_history[-1]:.4f}")
    ...
    def visualize_result(
        ...
        fig.suptitle(
            f"3D Reconstruction  |  IoU={final_iou:.4f}  "
            f"|  elev={best_elev}deg  azim={best_azim}deg",
        ...
        for ax, data, title, cmap in zip(
            ...
            ["Fossil mask", "Model projection",
             f"Difference (IoU={final_iou:.3f})",
             "Overlay (R=mask  G=projection)"],
            ["gray", "gray", "hot", None],
        ):
        ...
        p0 = phase1_end
        p1 = p0 + (len(loss_history) - p0) // 2
        ax4.plot(range(p0), loss_history[:p0],
                 color="#43A047", lw=1.5, label="Phase 0 (scale)")
        ax4.plot(range(p0, p1), loss_history[p0:p1],
                 color="#1565C0", lw=1.5, label="Phase 1 (coarse deformation)")
        ax4.plot(range(p1, len(loss_history)), loss_history[p1:],
                 color="#E65100", lw=1.5, label="Phase 2 (fine deformation)")
        ...
        ax4.set_title("IoU Loss: 3-phase optimization")
        ...
        table = ax5.table(
            cellText=[
                ...
                ["", ""],
                ["elev / azim",  f"{best_elev}deg / {best_azim}deg"],
...
class FossilApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(row1, text=f"(max {mp.cpu_count()})",
    ...
    def _pipeline(self):
        try:
            ...
            self._set_status(
                ...
                f"(elev={best_elev}deg, azim={best_azim}deg). Optimizing..."
            )
            ...
            self._set_status(
                f"Done! IoU={final_iou:.4f}  |  "
                f"Search: {search_time:.1f} s  "
                f"Opt.: {opt_time:.1f} s ({len(loss_history)} iters)  "
                f"Total: {total:.1f} s"
            )
